...
public class Main_Ubelix_25k_INSERT {
	...
	public static void main(String[] args) {
		Main_Ubelix_25k_INSERT start = new Main_Ubelix_25k_INSERT();
		
		ArrayList<AnalysisResult> ars = start.readZipFile();
		start.storeAnalysisResults(ars);
	}

	public Main_Ubelix_25k_INSERT() {
	...
	private void storeAnalysisResults(ArrayList<AnalysisResult> arl) {
		...
		for (AnalysisResult ar : arl) {
			dbh.insertVirusShareEntry(ar);
			arCount++;
			if (arCount % 1000 == 0) {
				System.out.println("Main: Inserted row " + arCount);
			}
		}
	}
}
